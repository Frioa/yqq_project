...
import com.yqq.lib_audio.event.AudioReleaseEvent;
import com.yqq.lib_audio.event.AudioStartEvent;
import com.yqq.lib_audio.model.AudioBean;
import com.yqq.lib_audio.view.NotificationHelper;
...
import static com.yqq.lib_audio.view.NotificationHelper.NOTIFICATION_ID;
...
public class MusicService extends Service implements NotificationHelper.NotificationHelperListener {
    ...
    public static void startMusicService(ArrayList<AudioBean> audioBeans) {
        ...
        Log.d(TAG, "startMusicService() " + intent);
    ...
    public void onNotificationInit() {
        // service与Notification绑定, 成为前台服务
        Log.d(TAG, " service与Notification绑定, 成为前台服务");
    ...
    // 为什么通过 Service 来通知 NotificationHelper 更新UI，不直接更新？
    // Notification 是运行再系统进程无法直接通信
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onAudioLoadEvent(AudioLoadEvent event) {
        //更新notifacation为load状态
        NotificationHelper.getInstance().showLoadStatus(event.getMAudioBean());
    }
